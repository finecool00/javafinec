package quiz22;

import java.util.Arrays;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class RegexQuiz02 {

	public static void main(String[] args) {
	
		String str = "123123-1231231 GS25(치킨마요) 4,400원";
		String str2 = "123123-1231232 GS25(주부18단) 5,000원";
		
		String pattern = "\\d{6}-\\d{7}";
		String pattern2 = "[A-Za-z]+25"; //GS25
		String pattern3 = "\\([가-힣0-9]+\\)";
		String pattern4 = "[0-9,]+원";
		
		List<String> list = Arrays.asList(str, str2);

		for(String s : list) {
			Matcher m = Pattern.compile(pattern).matcher(s);
			Matcher m2 = Pattern.compile(pattern2).matcher(s);
			Matcher m3 = Pattern.compile(pattern3).matcher(s);
			Matcher m4 = Pattern.compile(pattern4).matcher(s);
			
			if(m.find() && m2.find() && m3.find() && m4.find()) {
				System.out.println(m.group());
				System.out.println(m2.group());
				System.out.println(m3.group());
				System.out.println(m4.group());
			}
		
		
//		for(int i = 0; i < list.size(); i++) {
//			String str3 = list.get(i);
//			
//			Matcher w = Pattern.compile(pattern).matcher(str);
//			Matcher x = Pattern.compile(pattern2).matcher(str);
//			Matcher y = Pattern.compile(pattern3).matcher(str);
//			Matcher z = Pattern.compile(pattern4).matcher(str);
//			
//			if(w.find() && x.find() && y.find() && z.find()) {
//				System.out.println(w.group());
//				System.out.println(x.group());
//				System.out.println(y.group());
//				System.out.println(z.group());
//			}
		}
		
		
	}
	
	
	
	
	
	
	
	
}
